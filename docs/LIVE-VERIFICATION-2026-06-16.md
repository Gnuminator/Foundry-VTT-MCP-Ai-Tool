...
## Result: read surface healthy — 25/32 pass, 0 real failures
...
| measure-distance           | ⚠️ harness | "failed" only because the sweep fed it a single generic token arg; it correctly rejects malformed input. Needs a proper token/point pair — re-run by hand. |

...
| 3   | Foundry module `data-access` / `queries` | upstream-derived | 🟡 In progress | **Remnant removal done:** all non-dnd5e code (PF2e/DSA5/WFRP/Cosmere) stripped from `data-access.ts` — dedicated index builders/extractors, spell helpers, creature-index types, and system-gated branches; dnd5e paths untouched. −1,127 LOC (9,781 → 8,654). Remaining: reimplement/reorganize `data-access` + `queries` from the idea; Foundry-link frame-type adoption (`socket-bridge`/`webrtc-connection`, `import type` only per the bundler policy); transaction-manager rewrite.                                                                                                                                                                                                                                                                                                                                         |
...
- **2026-06-15 — Chunk 3 start (data-access remnant removal).** Stripped all non-dnd5e system code
  ...
  dispatch is now dnd5e-only (throws for other systems). **−1,127 LOC** (9,781 → 8,654);

...
| 4   | Tool layer, domain-by-domain             | upstream-derived | 🟡 In progress       | **Approach:** these are thin, already-clean wrappers (validate args → `foundryClient.query('foundry-mcp-bridge.*')` → shape result), so the win is **test coverage** (the tools had none) + light cleanup — blind from-scratch rewrites of clean files are churn. Reserve real rewrites for the genuinely-large/messy files (`character` 1034, `dnd5e/add-feature` 1038, `compendium` 1329, `quest-creation` 1380). **Done so far:** test suites for `movement`, `effects`, `resources`, `session-log`, `combat`, `diagnostics`, `loot` (7 tool domains, mocked `FoundryClient`); mcp-server tests wired into CI. **134 mcp-server tests.** Pattern proven to fan out cleanly to parallel Sonnet workers (each writes only its own `*.test.ts`, verifies via `vitest`, no build collisions). Remaining: the rest of the thin tools (scene-control, encounter, chat-log, scene, dice-roll, actor-creation, ownership, token-manipulation, map-generation, dnd5e/\*, campaign-management) get suites the same way; the large/messy files (`character`, `dnd5e/add-feature`, `compendium`, `quest-creation`) get real rewrites + tests. |
...
- `npm test -w @gnuminator/mcp-server` — 134 tests (lock + tool-layer suites)

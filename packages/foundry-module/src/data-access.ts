...
export class FoundryDataAccess {
  ...
  /**
   * Sanitize a Foundry data object for tool output (delegates to shared core).
   */
  ...
  /** Resolve a damage/roll target to an Actor (delegates to shared core). */
  private resolveTargetActor(identifier: string): any {
    return shared.resolveTargetActor(identifier);
  }

  private rollModeFor(isPublic: boolean | undefined): string {
    return shared.rollModeFor(isPublic);
  }
